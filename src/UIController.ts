...
const PANEL_Y = CANVAS_HEIGHT - 120;
...
export class UIController extends Container {
  ...
  private buildUI(): void {
    ...
    this.betText.x = 20;
    this.betText.y = PANEL_Y;
    this.addChild(this.betText);

    this.createButton('bet_down', '−', 120, PANEL_Y - 2, 30, 24, () => this.adjustBet(-5));
    this.createButton('bet_up', '+', 160, PANEL_Y - 2, 30, 24, () => this.adjustBet(5));
    ...
    this.createButton('start', 'START', CANVAS_WIDTH / 2 - 60, PANEL_Y + 30, 120, 36, () => {
      this.onStart?.(this._bet, this._selectedBubble);
    }, 0x22aa44);

    this.createButton('cashout', 'CASH OUT', CANVAS_WIDTH / 2 - 70, PANEL_Y + 30, 140, 36, () => {
      this.onCashout?.();
    }, 0x22aa44);
  ...
  showRunningUI(activeBubble: BubbleId): void {
    ...
    for (const id of BUBBLE_IDS) {
      ...
      this.setButtonVisible(`swap_${id}`, true);
    }
  ...
  updateSwapButtons(activeBubble: BubbleId, aliveBubbles: BubbleId[]): void {
    for (const id of BUBBLE_IDS) {
      const isAlive = aliveBubbles.includes(id);
      const isCurrent = id === activeBubble;
      this.setButtonVisible(`swap_${id}`, isAlive && !isCurrent);
      this.setButtonEnabled(`swap_${id}`, isAlive && !isCurrent);
    }
  }
